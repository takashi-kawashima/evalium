...
from evalium.dataset import Conversation
...
class LangSmithIntegration:
    ...
    def __init__(self):
        ...
        try:
            ...
            Client = getattr(_ls, "Client", None) or getattr(
                _ls, "LangSmithClient", None
            )
        except Exception:
            Client = None
    ...
    def try_send_to_langsmith(
        self, metadata: Dict[str, Any], df: pd.DataFrame, artifact_path: str
    ) -> bool:
        ...
        try:
            run = self.client.create_run(
                name=f"{metadata['config_name']}_shop{metadata['shop_id']}_{metadata['prompt_date']}",
                run_type="llm",
                inputs=metadata,
            )

            self.client.update_run(
                run_id=run.id, run_type="llm", outputs={"artifact_path": artifact_path}
            )
